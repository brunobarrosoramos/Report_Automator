...
def main():
    inicio = datetime.now()
    print(f"[{inicio:%Y-%m-%d %H:%M:%S}] Iniciando geração do relatório LG...")

    try:
        id_tarefa = gerar_relatorio()
        print(f" - Relatório solicitado. IdTarefa: {id_tarefa}")
        ...
        while tempo_total < TEMPO_MAXIMO:
            ...
            print(f" - Status: {status or 'N/A'} | Tempo: {tempo_total}s")

            if status == 3551 and url:
                print(f" - Relatório pronto! URL: {url[:80]}...")
                break
        ...
        print(f"✅ Arquivo CSV salvo em: {destino}")

    except Exception as e:
        print(f"❌ Erro: {e}")

    fim = datetime.now()
    print(f"[{fim:%Y-%m-%d %H:%M:%S}] Execução finalizada. Duração total: {(fim - inicio).seconds}s")
